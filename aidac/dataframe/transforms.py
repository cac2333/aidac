...
class SQLJoinTransform(SQLTransform):
    ...
    def columns(self):
        if not self._columns_:
            ...
            def __extractsrccols__(sourcecols, tableName, left_side):
                # used for assign suffix
                if left_side:
                    suffix = self._lsuffix_
                    my_cols = src1cols
                    other_cols = src2cols
                else:
                    suffix = self._rsuffix_
                    my_cols = src2cols
                    other_cols = src1cols
                ...
                for c in projcols.values():
                    if c.name in my_cols and c.name in other_cols and not(
                        ...
                        proj_name = c.name
                    pc = self._construct_col(sourcecols, c.name, proj_name, None, tableName)
            ...
            self._columns_ = collections.OrderedDict(
                __extractsrccols__(src1cols, self._source1_.table_name,True) + __extractsrccols__(src2cols, self._source2_.table_name, False))
        return self._columns_
...
class SQLAGG_Transform(SQLTransform):
    ...
    def __init__(self, source, func, collist, numeric_only=False):
        ...
        self.func = self._convert_agg_name(func)
        ...
        if not self._columns_:
            ...
            if self._is_dict_:
                for col in self.collist:
                    ...
                    operations = [self._convert_agg_name(self.collist[col])] if isinstance(self.collist[col], str) else \
                        [self._convert_agg_name(c) for c in self.collist[col]]
                    ...
                    for operation in operations:
                        ...
                        if in_type(column.dtype, NumericTypes) or not self.numeric_only:
                            columns[projcoln] = column
            else:
                for col in self.collist:
                    srccol, projcoln, coltransform = _get_proj_col_info(col)
                    column = self._construct_col(src_cols, srccol, projcoln, coltransform, str(source))
                    column.agg_func = self._convert_agg_name(self.func)
                    if in_type(column.dtype, NumericTypes) or not self.numeric_only:
                        columns[projcoln] = column
    ...
    def genSQL(self):
        ...
        for c in self.columns:
            ...
            col = self.columns[c]
            if col.agg_func:
                func_format = col.agg_func + '({})'
            else:
                func_format = '{}'

            projcoltxt = ((projcoltxt + ", ") if (projcoltxt) else '') + ((func_format.format(col.column_expr) if col.column_expr
            else func_format.format(col.srccol[0])) + " AS " + col.name)
